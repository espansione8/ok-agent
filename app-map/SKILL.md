...
Read this ENTIRE skill file top to bottom. Every file output must already reflect the conventions below. This is not a guideline. It is a hard stop.
...
- When a route or lib authenticates via a hardcoded secret, describe the mechanism and the file location, but redact the value. Wording pattern: "The [id] path segment is a hardcoded shared secret (value redacted — see <file>; must be rotated and moved to env)."
...
This skill is stack-agnostic: the procedure is identical for any language or framework (TypeScript, Python, Go, Rust, Java, Ruby, PHP, C#, …; front-end, back-end, or full-stack). Every framework-specific hint below is an EXAMPLE of where to look, never a requirement.
...
If `.plan/spec.json` already exists and this skill is invoked only to reflect a small code change (e.g. called by a companion coding-standards skill after a change touches something `spec.json` tracks):
...
Leave `map.html` untouched unless the user explicitly asks to regenerate the visual map.
...
1. Project identity
Read the project manifest — whichever exists: `package.json`, `pyproject.toml` / `requirements.txt`, `go.mod`, `Cargo.toml`, `build.gradle` / `pom.xml`, `Gemfile`, `composer.json`, `mix.exs`, `*.csproj` — for name, version, scripts, dependencies
Identify language, framework, styling/UI layer, database, auth, third-party integrations; project description from README or manifest
→ `spec.json`: `project`, `name`, `description`, `version`, `techStack`, `stackItems`

2. Routes & pages
Find every route/page/endpoint definition, whatever the framework. Examples: SvelteKit `+page.svelte` / `+page.server.ts` / `+server.ts`; Next.js `app/**/page.tsx` / `route.ts` or `pages/**`; Express / Fastify / Hono / Koa router registrations; Django / Flask / FastAPI views + URL configs; Rails `routes.rb` → controllers; Spring `@RequestMapping` / `@RestController`; Go `http.HandleFunc` / chi / gin
Per route: path, module/namespace/layout group, HTTP methods, auth guards, role restrictions
Map navigation flow: redirects, which routes link to which; middleware/guards, public vs. protected sections
...
→ `spec.json`: `routes[]` (one entry per route, with its outgoing `links[]`)

3. Components
List every component — or the project's equivalent view unit: React `.tsx`/`.jsx`, Vue `.vue`, Svelte `.svelte`, Angular components, Rails partials, Django/Jinja templates, Blade/ERB/Twig files
Directories vary: `src/components/`, `app/components/`, `views/`, `templates/`, `partials/`
Per component: file path, what it does, live vs. dormant/legacy, shared vs. page-specific
→ `spec.json`: `components[]`

4. State management
Find all stores / state files: Redux slices, Zustand stores, Pinia/Vuex, Svelte stores, MobX, signals/composables, React context providers, NgRx/RxJS services; directories like `src/stores/`, `src/state/`, `store/`
Per store: file path, exported symbols, state held, consumers
Backend-only projects: capture the equivalent — shared singletons, caches, in-memory registries
→ `spec.json`: `stores[]`

5. Server-side modules
Middleware, config, service layer, background jobs/workers
Auth config, database client setup, ORM setup
...
6. Database schema
Find all schema/model/entity/migration files: Drizzle/Prisma schema files, SQLAlchemy/Django models, ActiveRecord models + migrations, Mongoose schemas, GORM structs, raw SQL migrations
...
Domain rules: credit/pricing rules, booking/reservation rules, role hierarchy, visibility scoping, notification rules, rate limiting
...
Scan `.env.example`, `.env`, config schema/validation files, or grep for env access: `process.env`, `import.meta.env`, `$env/`, `os.environ` / `os.getenv`, `os.Getenv`, `System.getenv`, `ENV[...]`, `Environment.GetEnvironmentVariable`
...
9. Scripts & commands
Every runnable command: package.json scripts, Makefile targets, poetry/uv scripts, cargo aliases, mix tasks, docker-compose commands
...

{
 ...
 "techStack": {
 "language": "TypeScript / Python / Go / Rust / Java / Ruby / ...",
 "framework": "SvelteKit 2 / Next.js 15 / Django 5 / Rails 7 / Spring Boot 3 / Gin / ...",
 "runtime": "front-end runtime or UI layer if any — Svelte 5 / React 19 / Vue 3 / templates / none",
 "styling": "Tailwind CSS 4 / CSS modules / SCSS / none",
 "database": "PostgreSQL / SQLite / MongoDB / MySQL / ...",
 "orm": "Drizzle / Prisma / SQLAlchemy / GORM / Ecto / Mongoose / none",
 "auth": "custom session cookie / next-auth / Devise / Spring Security / OAuth / ...",
 "adapter": "deploy target — node / vercel / docker / static / bare metal / ...",
 "...": "add any other stack facts worth remembering (queue, cache, deploy target)"
 },
 "stackItems": [
 { "name": "Express", "version": "5", "tag": "framework" },
 { "name": "Prisma", "tag": "orm" }
 ],
 "routes": [
 {
 ...
 "path": "/login",
 "methods": "GET·POST",
 "file": "src/routes/login/handler.ts (example — the project's real path)",
 "group": "(login) — route group / module / namespace; framework-dependent, '' if none",
 "accent": "auth",
 "dotColor": null,
 "tags": ["AUTH", "PUBLIC"],
 ...
 {
 "id": "C-01",
 "name": "Modal.tsx (example — .vue/.svelte/.erb/.html as applicable)",
 "file": "src/components/Modal.tsx",
 "status": "live",
 "description": "What it renders, its props/events/inputs, and where it's used."
 }
 ],
 "stores": [
 {
 ...
 "utils": [
 { "name": "utils", "file": "src/utils.ts", "functions": ["fn1", "fn2"] }
 ],
 "integrations": {
 "mailer": {
 "file": "src/server/mailer.ts",
 "functions": ["sendMail"],
 "description": "Email wrapper — SMTP config from env, used by mail-sending routes."
 }
 },
 "database": {
 ...
 "auth": {
 "provider": "custom session cookie / next-auth / Devise / ...",
 "methods": ["password"],
 "session": "sessionId cookie, validated in auth middleware (name the file)",
 "passwordHash": "bcrypt / argon2 / SHA-256 custom / ...",
 "guards": [
 { "scope": "dashboard group", "rule": "auth guard (src/lib/auth.ts) redirects to /login when no valid session" }
 ]
 },
 "businessLogic": {
 "key_concept": { "rule": "Precise description of the rule, including edge cases and where it's enforced (file/function)." },
 "security_notes": { "rule": "One entry per hardcoded secret found in the codebase: its location and purpose, value REDACTED, flagged 'rotate and move to env'. Example: 'Report CSV endpoint authenticates via a hardcoded shared secret in the [id] path segment (src/routes/api/report/[id]/handler.ts) — value redacted; rotate and externalize.'" }
 },
 "environmentVariables": [
 { "name": "DB_TYPE", "required": true, "topic": "core", "description": "local | remote | sync" },
 { "name": "DB_AUTH_TOKEN", "required": true, "topic": "core", "description": "needed if remote", "conditional": true },
 { "name": "MAILER_SECURE", "required": false, "topic": "mailer", "description": "force TLS" }
 ],
 "scripts": {
 "development": [ { "cmd": "npm run dev (or: make dev / cargo run / mix phx.server)", "description": "dev server" } ],
 "database": [ { "cmd": "npm run db:update", "description": "apply schema changes" } ],
 "buildAndTest": [ { "cmd": "npm run build", "description": "production build" } ]
 },
 "topology": {
 "zones": [
...
The inspector drawer (`<aside class="insp">`) works automatically off the same `data-*` attributes and the `EDGES` array — do not hand-wire it.
HTML-escape EVERY project-derived string before writing it into `map.html` — both inside `data-*` attribute values and in visible text (`.route`, `.file`, `.desc`, `.cdesc`, `.sdesc`, `.cols`, `.vd`, `.cmdline`, zone labels). Minimum escaping: `&` → `&amp;`, `<` → `&lt;`, `>` → `&gt;`, and inside attribute values `"` → `&quot;`. Placeholders like `<folderName>` / `<fileName>` must be written `&lt;folderName&gt;` / `&lt;fileName&gt;` — unescaped, the browser parses them as phantom start tags, the placeholders vanish from the rendered text, and the following text gets nested inside invisible elements.
Sections 02–05 are 1:1 renderings of the `spec.json` arrays — iterate the FULL array for each section, never hand-pick entries. Every `components[]` entry → one `.ccard`; every `stores[]` entry → one `.scard` in the stores grid; every `serverLibs[]` entry → one `.scard` in the server libs grid; every `database.tables[]` entry → one `.tcard`; every `database.relationships[]` entry → one `.rel .r`; every `environmentVariables[]` entry → one `.envrow`; every `stackItems[]` entry → one `.stk`. No entry may be dropped.
Every count printed in `map.html` — the masthead `.sheetmeta` line, each section's `.statrow` numbers, and each `.sec-sub` tally (e.g. "N live · M dormant") — must be computed from the same `spec.json` arrays it renders. The Section 02 live/dormant split must equal the `status` values in `components[]`; the Section 01 stat counters must equal the actual route `kind`/`accent` distribution. Never copy counts by eye — recount after the final card list is written.
SECRETS GATE applies to `map.html` too: no secret value may appear in any `data-desc`, `.desc`, `.file`, `.cols`, or card text — the same redacted wording as in `spec.json`.
...
<section id="s01">
  <div class="sec-head">
    ...
    <!-- ADAPT: where routes are defined, e.g. src/routes/** · app/** · routers/ · urls.py -->
    <span class="crumb">src/routes/**</span>
    <!-- ADAPT: counts — COMPUTED from routes[]: total route files, public, protected, api rails -->
    <div class="statrow">
    ...
    <div class="topo-canvas" id="canvas">
      ...
      <div class="zone hi">
        ...
        <!-- ADAPT: one .rcard per route in this zone's columns (flattened).
             ...
             object, HTML-ESCAPED (& < > " — see critical rules; never a literal secret value).
...
<section id="s02" class="reveal">
  <div class="sec-head"><span class="num">SECTION 02</span><!-- ADAPT: components/views dir(s), e.g. src/components/ · views/ · templates/ --><span class="crumb">src/components/</span></div>
  <h2 class="sec-title">Component Ledger</h2>
  <!-- ADAPT: counts — COMPUTED by walking components[]: status:"live" vs status:"dormant".
  ...
  <div class="grid-c">
    ...
    <div class="ccard"><span class="pill">LIVE</span><div class="cid">C‑01</div><div class="cname">Component.ext</div><div class="cpath">src/components/</div><div class="cdesc">What it does.</div></div>
    <div class="ccard dorm"><span class="pill">DORMANT</span><div class="cid">C‑02</div><div class="cname">Unused.ext</div><div class="cpath">src/components/</div><div class="cdesc">Why it's dormant / no longer referenced.</div></div>
  </div>
</section>

<!-- ===================== SECTION 03 — STATE & SERVER ===================== -->
<section id="s03" class="reveal">
  <div class="sec-head"><span class="num">SECTION 03</span><!-- ADAPT: state + server dirs --><span class="crumb">src/stores/ · src/server/</span></div>
...
<section id="s05" class="reveal">
  ...
  <div class="envcard">
    <div class="envhead">
      ...
      <!-- ADAPT: env source, e.g. .env · settings.py · application.yml -->
...
Analyze the project with the 9-step checklist. Parallel file-pickers/code-searchers; read key files. The procedure is stack-agnostic — apply the steps to whatever language/framework the project actually uses.
Produce `spec.json` per the schema — every route (with its `links`), table, column, component, store, server lib, integration, business rule, env var. It must stand alone: another agent reading only `spec.json` can find the right file and make a correct change without re-scanning.
Produce `map.html` — copy the CSS and JS blocks verbatim from the template; build Section 01 topology from `spec.json → topology.zones` (plain flexbox zones/columns — no manual coordinate math); flatten `routes[].links` into the `EDGES` array; fill Sections 02–05 straight from the matching `spec.json` arrays per the ADAPT comments — iterating each array in full.
Validate before finishing:
(a) Interactivity — open `map.html` in a browser: topology interactive (hover = trace wires, click = inspector drawer, Escape AND the ✕ button both close the drawer), all 5 sections render, every `EDGES` entry references two ids that exist as `.rcard` elements on the page.
(b) Completeness — cross-check rendered cards against `spec.json`: `.rcard` count == `routes[]` length; `.ccard` count == `components[]` length; stores-grid `.scard` count == `stores[]` length; server-grid `.scard` count == `serverLibs[]` length; `.tcard` count == `database.tables[]` length; `.rel .r` count == `database.relationships[]` length; `.envrow` count == `environmentVariables[]` length; `.stk` count == `stackItems[]` length. Every printed count (sheetmeta, statrow, sec-sub — including the Section 02 live/dormant split) must match the arrays exactly. Fix any drift — a card dropped or a stale tally is a failed validation.
(c) Secrets + escaping — grep the final `spec.json` and `map.html` for any literal secret value found in the codebase: it must appear nowhere, only the redacted wording. Confirm no unescaped `<placeholder>` text survived into HTML (search for suspiciously-short rendered `.desc` text if unsure).