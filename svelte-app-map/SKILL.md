...
### 1. Project identity
- Read `package.json` / `Cargo.toml` / `requirements.txt` / `build.gradle` for name, version, scripts, dependencies
- Identify framework, styling, database, auth, third-party integrations; project description from README or package.json
- → `spec.json`: `project`, `name`, `description`, `version`, `techStack`, `stackItems`

### 2. Routes & pages
- Find every route/page file (e.g. `+page.svelte`, `+page.server.ts`, `+server.ts`, `+layout.svelte`)
...
### 3. Components
- List every component in the components directory (e.g. `src/lib/components/`, `src/components/`)
- Per component: file path, what it does, live vs. dormant/legacy, shared vs. layout-specific
- → `spec.json`: `components[]`

### 4. State management
- Find all stores / state files (e.g. `src/lib/stores/`, `src/store/`)
...
### 6. Database schema
- Find all schema/model/entity files
...
- Scan `.env.example`, `.env`, schema files, or grep for `process.env` / `$env/`
...
### 9. Scripts & commands
- Every script from package.json (or equivalent)
